...
function ItemWrapper({ children }) {
    ...
    return (
        <div
            ...
            className={`border-grey-04 text-grey-02 bg-black-01 flex rounded-sm border hover:brightness-110 ${maximize && 'absolute inset-0 mx-auto h-max w-[clamp(100%,50%,10%)]'}`}
        >
            {children}
        </div>
    )
